...
def add_entry(date, content, weather, health, rating, activities=None, mood=None, memo=None, sleep_hours=None, category=None):  
    diary = load_diary()  
...
def diary_form():  
    ...
    category = existing_entry.get("category", "その他") if existing_entry else "その他"  
    ...
    with col1:  
        ...
        health = st.selectbox("😷 体調", ["元気", "普通", "少し疲れた", "体調不良", "絶好調", "眠い"], index=["元気", "普通", "少し疲れた", "体調不良", "絶好調", "眠い"].index(health))  
  
    with col2:  
        mood = st.selectbox("🧠 気分", ["選択しない", "幸せ", "充実", "退屈", "不安", "悲しい", "イライラ", "やる気満々", "リラックス", "達成感"], index=["選択しない", "幸せ", "充実", "退屈", "不安", "悲しい", "イライラ", "やる気満々", "リラックス", "達成感"].index(mood))  
    ...
    category = st.selectbox("🗂 カテゴリー", ["仕事", "プライベート", "健康", "その他"], index=["仕事", "プライベート", "健康", "その他"].index(category))  
  
    # 活動タグ  
    activity_options = ["運動した", "読書した", "料理した", "友達と会った", "家族と過ごした", "勉強した", "映画/TVを見た", "創作活動をした", "ゲームをした", "休息した", "仕事/バイトをした", "新しいことを学んだ"]  
    ...
    if st.button("💾 保存する", key="save_diary"):  
        if content.strip():  
            add_entry(selected_date, content, weather, health, rating, selected_activities, mood, memo, sleep_hours, category)  
            st.success(f"✅ {selected_date} の日記を保存しました！")  
            st.balloons()  
            auto_tagging(load_diary())  
        else:  
        ...
        for i, day in enumerate(week):  
            if day == 0:  
                ...
                if not day_data.empty:  
                    ...
                    <div>  
                        <p>🌤 天気: {weather}</p>  
                        <p>😷 体調: {entry.get('health', '')}</p>  
                        <p>📝 {entry.get('content', '').replace('\n', '<br>')}</p>  
                    </div>  
                    """  
                    cols[i].markdown(f"""  
                    <div style='text-align: center; padding: 5px; background-color: rgba(144, 238, 144, 0.2); border-radius: 5px; cursor: pointer;'   
                         onclick="alert('{popup_content}')">  
...
def show_statistics():  
    ...
    with tabs[0]:  
        ...
        fig = px.line(df, x="date", y="rating", title="日々の評価の推移", labels={"rating": "評価", "date": "日付"}, markers=True)  
        ...
        if selected_points:  
            ...
            entry = get_entry_by_date(selected_date.strftime("%Y-%m-%d"))  
            st.write(f"📅 **{selected_date.strftime('%Y-%m-%d')}**")  
            st.write(f"🌤 天気: {entry['weather']}")  
            st.write(f"😷 体調: {entry['health']}")  
...
def export_to_csv(diary_data):  
    ...
    df = df[["date", "content", "weather", "health", "rating", "activities", "mood", "memo", "sleep_hours", "category"]]  
    return df.to_csv(index=False).encode('utf-8-sig')  
  
# 自動タグ付け機能  
def auto_tagging(diary):  
    vectorizer = TfidfVectorizer(stop_words='english')  
    X = vectorizer.fit_transform([entry['content'] for entry in diary])  
